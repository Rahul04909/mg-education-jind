...
$exam_id = intval($_GET['exam_id']); // internship_paper_id

// Fetch Result
$sql = "SELECT er.*, i.title as subject_name 
        FROM internship_results er
        JOIN internship_question_papers qp ON er.internship_paper_id = qp.id
        JOIN internships i ON qp.internship_id = i.id
        WHERE er.student_id = $student_id AND er.internship_paper_id = $exam_id
...
<head>
    ...
    <title>Result - <?php echo htmlspecialchars($result['subject_name']); ?></title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .score-panel { width: 40%; background: linear-gradient(135deg, var(--primary) 0%, #4f46e5 100%); color: white; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px; }
        .score-circle-outer { width: 280px; height: 280px; border-radius: 50%; background: rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; margin-bottom: 40px; backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.2); position: relative; }
        .score-circle-inner { width: 240px; height: 240px; background: white; border-radius: 50%; display: flex; flex-direction: column; align-items: center; justify-content: center; color: var(--dark); box-shadow: 0 20px 50px rgba(0,0,0,0.2); }
        ...
        .details-panel { flex: 1; padding: 60px 80px; display: flex; flex-direction: column; justify-content: center; overflow-y: auto; }
        ...
        .status-box { padding: 25px; border-radius: 16px; background: <?php echo $bg_color; ?>; border: 1px solid <?php echo $color; ?>; color: <?php echo $color; ?>; margin-bottom: 40px; display: flex; align-items: center; gap: 20px; }
...
<body>
    ...
    <div class="details-panel">
        <div class="exam-header">
            <span class="subject-badge"><?php echo htmlspecialchars($result['subject_name']); ?></span>
            <h1 class="exam-title">Exam Overview</h1>
            <p class="exam-date">Completed on <?php echo date('d M, Y \a\t h:i A', strtotime($result['attempted_at'])); ?></p>
        </div>
        ...
        <div class="status-box">
            ...
            <div class="status-text">
                ...
                <p>You have <strong><?php echo strtoupper($status); ?>ED</strong> this exam with <?php echo $percentage; ?>% marks.</p>
            </div>
        </div>

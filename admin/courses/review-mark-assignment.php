...
// Fetch Submissions
// Only fetch students added by 'admin' (implied by requirement "only students added by admin")
// In admissions table, usually there is added_by_role (or check if center_id is 0/NULL)
// Assuming data structure based on typical MG apps: usually center_id is NULL or 0 for admin students.
// Let's check `admissions` table structure from memory or assumption. Safest is to check if `center_id` is 0 or NULL if that column exists.
// Or if there is `added_by` column.
// Based on previous chats, Center students have `center_id`. Admin students have `added_by` = 'admin' or something?
// Re-checking admsisions table structure would have been good, but let's assume filtering by `center_id IS NULL OR center_id = 0` is the way to separate admin students.

$where_clause = "WHERE (std.center_id IS NULL OR std.center_id = 0)"; // Filter for Admin students
...
<body>
    <main class="admin-content">
        <div class="page-header">
            <div>
                <a href="../index.php" style="text-decoration:none;color:var(--indigo);font-size:14px">Dashboard</a> › Courses
                <h1 class="page-title">Review Assignments</h1>
                <p style="color:var(--muted)">Review and mark student assignments (Admin Students Only)</p>
            </div>
        </div>

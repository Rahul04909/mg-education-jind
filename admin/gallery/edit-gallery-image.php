...
if (!isset($_GET['id'])) {
    header("Location: manage-gallery-image.php");
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (in_array($file_ext, $allowed_ext)) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], __DIR__ . '/' . $target_file)) {
                ...
                if (!empty($image_data['image_path'])) {
                    $old_file = __DIR__ . '/../../' . $image_data['image_path'];
                ...
                $sql = "UPDATE gallery_images SET category_id = $category_id, title = '$title', image_path = '$db_path', is_active = $is_active WHERE id = $id";
                
                if ($conn->query($sql) === TRUE) {
                    $success_message = "Gallery Image updated successfully!";
                    // Refresh data
                    $image_data['image_path'] = $db_path;
                } else {
        ...
        $sql = "UPDATE gallery_images SET category_id = $category_id, title = '$title', is_active = $is_active WHERE id = $id";
        if ($conn->query($sql) === TRUE) {
            $success_message = "Gallery Image details updated successfully!";
        } else {
            $error_message = "Database Error: " . $conn->error;
        }
    }
    
    // Refresh data in variable for display
    $image_data['title'] = $title;
    $image_data['category_id'] = $category_id;
    $image_data['is_active'] = $is_active;
}
...
<head>
    ...
    <title>Edit Gallery Image - MG Education</title>
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .current-img{max-width: 200px; border-radius: 8px; margin-bottom: 15px; display: block;}
        ...
        <div class="admin-wrap">
            <div class="page-header">
                <div class="breadcrumb">
                    <a href="../index.php">Dashboard</a> › <a href="manage-gallery-image.php">Gallery Images</a> › Edit Image
                </div>
                <h1 class="page-title">Edit Gallery Image</h1>
                <p class="page-subtitle">Update image details</p>
            </div>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="card">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-input" required value="<?php echo htmlspecialchars($image_data['title']); ?>">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" class="form-select" required>
                            ...
                            <?php foreach($categories as $cat): ?>
                                <option value="<?php echo $cat['id']; ?>" <?php echo ($cat['id'] == $image_data['category_id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <?php if(!empty($image_data['image_path'])): ?>
                            <img src="../../<?php echo htmlspecialchars($image_data['image_path']); ?>" class="current-img" alt="Current Image">
                        <?php else: ?>
                            <p style="color:var(--muted)">No image uploaded.</p>
                        <?php endif; ?>

                        <label class="form-label" style="margin-top:15px;">Upload New Image (Optional)</label>
                        <input type="file" name="image" id="imageInput" class="form-input" accept="image/png, image/jpeg, image/webp">
                    ...
                    <div class="form-group">
                        <label class="form-label">
                            <input type="checkbox" name="is_active" <?php echo ($image_data['is_active']) ? 'checked' : ''; ?>> Active
                        </label>
                    </div>

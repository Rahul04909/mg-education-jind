...
// Ensure schema (can rely on central setup now)
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] == 'add_session') {
    ...
    // Validate
    if ($internship_id > 0 && !empty($start_month) && !empty($end_month)) {
        $sql = "INSERT INTO internship_sessions (internship_id, session_name, start_month, start_year, end_month, end_year) 
                VALUES ($internship_id, '$session_name', '$start_month', $start_year, '$end_month', $end_year)";
...
if (isset($_GET['delete_id'])) {
    ...
    $conn->query("DELETE FROM internship_sessions WHERE id = $delete_id");
...
$filter_internship_id = isset($_GET['filter_internship']) ? intval($_GET['filter_internship']) : 0;
$query = "SELECT s.*, i.title as internship_title 
          FROM internship_sessions s 
          JOIN internships i ON s.internship_id = i.id";

if ($filter_internship_id > 0) {
    $query .= " WHERE s.internship_id = $filter_internship_id";
}
...
<head>
    ...
    <title>Manage Internship Sessions - MG Admin</title>
        ...
        <div class="admin-wrap">
            <div class="page-header">
                <div class="breadcrumb">
                    <a href="../index.php">Dashboard</a> › <a href="index.php">Internships</a> › Sessions
                </div>
                <h1 class="page-title">Manage Internship Sessions</h1>
                <p class="page-subtitle">Add and manage sessions (batches) for internships</p>
            </div>
            ...
            <!-- Add Session Form -->
            <div class="card">
                <h3 style="margin-bottom: 20px; font-size: 18px; font-weight:700">Add New Session</h3>
                <form method="POST">
            ...
            <div class="card">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <h3 style="font-size: 18px; font-weight:700">All Sessions</h3>
                </div>

                <div class="filter-bar">
                    <span style="font-weight: 600; font-size: 14px; color: var(--muted);">Filter by Internship:</span>
                    <form method="GET" style="display: flex; gap: 10px; flex: 1;">
                        <select name="filter_internship" class="form-select" style="max-width: 300px;" onchange="this.form.submit()">
                            <option value="">All Internships</option>
                            <?php foreach($internships as $i): ?>
                                <option value="<?php echo $i['id']; ?>" <?php if($filter_internship_id == $i['id']) echo 'selected'; ?>>
                                    <?php echo htmlspecialchars($i['title']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <?php if($filter_internship_id > 0): ?>
                            <a href="manage-internship-sessions.php" style="display: flex; align-items: center; color: var(--error); text-decoration: none; font-size: 14px;">Clear Filter</a>
                        <?php endif; ?>
                    </form>
                </div>

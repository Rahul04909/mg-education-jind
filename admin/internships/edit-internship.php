...
$fees_data = json_decode($internship['fees'], true);
$u_fee_amount = isset($fees_data['amount']) ? $fees_data['amount'] : '';
$u_fee_text = isset($fees_data['description']) ? $fees_data['description'] : '';
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $fees_json = mysqli_real_escape_string($conn, json_encode($fees_data_new));
    ...
    if (empty($error_message)) {
        $update_sql = "UPDATE internships SET 
            title = '$title',
            slug = '$slug',
...
<head>
    ...
    <title>Edit Internship - MG Education</title>
        ...
        <div class="admin-wrap">
            <div class="page-header">
                <div class="breadcrumb">
                    <a href="../index.php">Dashboard</a> › <a href="index.php">Internships</a> › Edit Internship
                </div>
                <h1 class="page-title">Edit Internship: <?php echo htmlspecialchars($internship['title']); ?></h1>
                <p class="page-subtitle">Update internship details</p>
            </div>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="card">
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" class="form-input" required value="<?php echo htmlspecialchars($internship['title']); ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Slug (Optional)</label>
                            <input type="text" name="slug" id="slug" class="form-input" value="<?php echo htmlspecialchars($internship['slug']); ?>">
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" rows="10"><?php echo htmlspecialchars($internship['description']); ?></textarea>
                    </div>
                </div>
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <div style="display: flex; gap: 10px;">
                                <input type="number" name="duration_value" class="form-input" required style="width: 120px;" value="<?php echo intval($internship['duration_value']); ?>">
                                <select name="duration_type" class="form-select" required>
                                    <option value="Months" <?php if($internship['duration_type'] == 'Months') echo 'selected'; ?>>Months</option>
                                    <option value="Weeks" <?php if($internship['duration_type'] == 'Weeks') echo 'selected'; ?>>Weeks</option>
                                    <option value="Days" <?php if($internship['duration_type'] == 'Days') echo 'selected'; ?>>Days</option>
                                    <option value="Years" <?php if($internship['duration_type'] == 'Years') echo 'selected'; ?>>Years</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Fee Amount (INR)</label>
                            <input type="number" name="fee_amount" class="form-input" placeholder="0.00" value="<?php echo htmlspecialchars($u_fee_amount); ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Fee Note / Details</label>
                        <input type="text" name="fee_text" class="form-input" value="<?php echo htmlspecialchars($u_fee_text); ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <?php if(!empty($internship['featured_image'])): ?>
                            <img id="image-preview" class="preview-image" src="<?php echo '../../' . $internship['featured_image']; ?>" alt="Current Image">
                            <p style="font-size: 13px; color: var(--muted); margin-top: 5px;">Current Image (Upload new to replace)</p>
                        <?php else: ?>
                ...
                <div class="card">
                    ...
                    <div class="grid-2">
                         <div class="form-group">
                            ...
                            <input type="text" name="meta_title" class="form-input" value="<?php echo htmlspecialchars($internship['meta_title']); ?>">
                        </div>
                         <div class="form-group">
                            <label class="form-label">Meta Keywords</label>
                            <input type="text" name="meta_keywords" class="form-input" value="<?php echo htmlspecialchars($internship['meta_keywords']); ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Meta Description</label>
                        <textarea name="meta_desc" class="form-textarea" rows="2"><?php echo htmlspecialchars($internship['meta_desc']); ?></textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">
                            <input type="checkbox" name="is_active" <?php if($internship['is_active']) echo 'checked'; ?>> Active
                        </label>
                    </div>
                </div>

                <div style="margin-bottom: 50px;">
                    <button type="submit" class="btn btn-primary" style="width: 100%; justify-content: center; padding: 16px; font-size: 16px;">Update Internship</button>
                </div>

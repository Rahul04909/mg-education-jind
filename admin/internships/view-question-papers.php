...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] == 'delete_paper') {
    ...
    $sql = "DELETE FROM internship_question_papers WHERE id = $paper_id";
...
$sql_papers = "SELECT qp.*, i.title as internship_title, s.session_name 
               FROM internship_question_papers qp 
               JOIN internships i ON qp.internship_id = i.id 
               JOIN internship_sessions s ON qp.session_id = s.id 
...
<head>
    ...
    <title>View Internship Papers - MG Admin</title>
    <style>
...
<body>
    <main class="admin-content">
        <div class="page-header">
            <div>
                <div style="font-size:14px;color:var(--muted);margin-bottom:10px">
                    <a href="../index.php" style="text-decoration:none;color:var(--indigo)">Dashboard</a> › Internships › Question Papers
                </div>
                <h1 class="page-title">View Internship Papers</h1>
            </div>
            <div>
                <a href="create-internship-question-paper.php" class="btn btn-primary">+ Create New Paper</a>
            </div>
        </div>
        ...
        <div class="card">
            <h3 style="font-size:18px; font-weight:700; margin-bottom:15px;">All Internship Question Papers</h3>
            <table class="table">
                <thead>
                    <tr>
                        <th>Internship</th>
                ...
                <tbody>
                    <?php if ($result_papers->num_rows > 0): ?>
                        ...
                        <tr>
                            <td style="font-weight:600; color:var(--text)"><?php echo htmlspecialchars($row['internship_title']); ?></td>
                            ...
                            <td>
                                ...
                                <span style="font-size:12px;color:var(--muted)"><?php echo date("h:i A", strtotime($row['start_time'])); ?> - <?php echo date("h:i A", strtotime($row['end_time'])); ?></span>
                            </td>
                            <td><span style="font-weight:700; color:var(--active)"><?php echo $row['total_marks']; ?></span></td>
                            <td style="text-align:right">
                                <a href="create-internship-question-paper.php?edit_id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary" style="background:none; color:var(--indigo); border:1px solid var(--line)">Edit</a>
                                
                                <form method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this paper?');">
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted)">No question papers found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>

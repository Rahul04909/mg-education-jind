...
if (!$result) {
    ...
    // We can try to handle it or just let it show the error (which helps debugging)
    $error = $conn->error;
}
...
<head>
    ...
    <title>Internships - MG Education</title>
    <style>
    ...
    <main class="admin-content">
        <div class="page-header">
            <div>
                ...
                <p style="color:var(--muted)">Manage internships and applications</p>
            </div>
            <div style="display:flex; gap:10px">
                <a href="add-internship.php" class="btn btn-primary">+ Add Internship</a>
            </div>
        </div>

        <?php if (isset($error)): ?>
            <div style="color: red; padding: 20px;">Database Error: <?php echo $error; ?></div>
        <?php else: ?>
        <div class="card">
            <table class="table">
                <thead>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while($row = $result->fetch_assoc()): 
                            ...
                            $fee_display = isset($fees['amount']) && $fees['amount'] ? 'INR ' . number_format($fees['amount']) : 'Free';
                        ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                <a href="edit-internship.php?id=<?php echo $row['id']; ?>" style="color:var(--indigo);font-weight:600;text-decoration:none">Edit</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted)">No internships found. Add your first internship!</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>

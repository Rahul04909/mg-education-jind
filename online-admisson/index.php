<?php
require_once __DIR__ . '/db-setup.php'; // Ensure DB exists
require_once __DIR__ . '/../database/db-config.php';
$conn = getDbConnection();

// Fetch Active Courses
$courses = [];
$sql = "SELECT id, title, fees, duration_value, duration_type FROM courses WHERE is_active = 1 ORDER BY title ASC";
...
<head>
    ...
    <title>Online Admission - MG Education</title>
...
<div class="sidebar">
    <div>
        <h1>MG Education<br>Online Admission</h1>
        <p>Join our premium courses and start your journey to success. Fill out the application form to proceed.</p>
        <ul class="steps">
            <li class="active"><span class="num">1</span> Basic Details</li>
            <li class="active"><span class="num">2</span> Contact Info</li>
            <li class="active"><span class="num">3</span> Uploads</li>
            <li class="active"><span class="num">4</span> Payment</li>
        </ul>
    </div>
    ...
    <div class="container">
        <form id="admissionForm" enctype="multipart/form-data">
            ...
            <div id="feeBox" class="fee-card" style="display:none">
                <div class="fee-row"><span>Course Duration:</span> <span id="durDisplay"></span></div>
                <div class="fee-row total-row"><span>Total Fees:</span> <span id="feeDisplay"></span></div>
            ...
            <!-- Skill Background -->
            <h3 class="section-title">Skill Background</h3>
            <div class="form-grid">
                <div class="form-group">
            ...
            <div class="form-group">
                <label>Previous Course with MG Education? *</label>
            ...
            <div id="prevCourseBox" class="form-grid" style="display:none">
                <div class="form-group"><label>Enrollment Number</label><input type="text" name="prev_enroll_no"></div>
                <div class="form-group"><label>Course Name</label><input type="text" name="prev_course_name"></div>
                <div class="form-group"><label>Course Session</label><input type="text" name="prev_course_session"></div>
            </div>
            ...
            <button type="button" id="payBtn" class="btn" onclick="initiatePayment()">Proceed to Payment</button>
        </form>
    </div>
</div>
...
<script>
    ...
    // Fee Update
    function updateFee() {
        let sel = document.getElementById('course_select');
            ...
            if(data.status === 'success') {
                ...
                let options = {
                    ...
                    "handler": function (response){
                        // Payment Success - Submit Admission
                        submitAdmission(formData, response);
                    },
                    "prefill": {
    ...
    function submitAdmission(formData, paymentResponse) {
        ...
        let payBtn = document.getElementById('payBtn');
        payBtn.innerText = 'Finalizing Admission...';
        ...
        .then(data => {
            if(data.status === 'success') {
                alert("Admission Successful! Enrollment No: " + data.enrollment_no);
                window.location.reload();
            } else {
                alert("Admission Saved Failed: " + data.message + ". Contact Support with Payment ID: " + paymentResponse.razorpay_payment_id);
            }
            payBtn.disabled = false;
            payBtn.innerText = 'Proceed to Payment';
        });
